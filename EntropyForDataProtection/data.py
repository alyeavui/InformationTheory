...
def generate_low_entropy_plaintext(length: int) -> str:
    return 'A' * length  # Repeated pattern to reduce entropy
...
def aes_encrypt(plaintext: str) -> bytes:
    ...
    padded_plaintext = plaintext.ljust((len(plaintext) + 15) // 16 * 16)  # Pad to 16 bytes
...
def generate_dataset(num_samples: int) -> List[Tuple[bytes, int]]:
    ...
    for _ in range(num_samples // 2):
        ...
        for encrypt_func in [aes_encrypt, rsa_encrypt, chacha20_encrypt]:
            ...
            dataset.append((encrypted_data, 1))  # High entropy = Secure (label 1)

            encrypted_data_low = encrypt_func(low_entropy_plaintext)
            entropy_value_low = calculate_entropy(encrypted_data_low)
            dataset.append((encrypted_data_low, 0))  # Low entropy = Weak (label 0)
    return dataset

# Generate dataset
dataset = generate_dataset(100)

# Save dataset for training the model
import pickle
